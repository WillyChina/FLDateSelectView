...
#ifndef isIPhone5
#define isIPhone5  (self.frame.size.width == 320)
#endif
...
//        self.frame = CGRectMake(self.frame.origin.x, self.frame.origin.y, 320, 216);
    if (self.frame.size.height != 216 && self.frame.size.height != 150)
    {
        self.height = 40;
        UIView *firstHeaderView = [self createHeaderViewWithType:1];
    ...
    if (!myPickerView) {
        myPickerView = [[UIPickerView alloc] initWithFrame:CGRectMake(0, self.height, self.frame.size.width, self.frame.size.height-self.height)];
        CGRect frame = self.frame;
        if (frame.size.height < 216+self.height) {
            frame.size.height = 216+self.height;
        }
        if (self.frame.size.width < 320) {
            frame.size.width = 320;
        }
        if (frame.size.width > 375) {
            frame.origin.x = (frame.size.width - 375) / 2;
            frame.size.width = 375;
        }
        myPickerView.frame = CGRectMake(0, self.height, self.frame.size.width, self.frame.size.height-self.height);
...
- (UIView *)createHeaderViewWithType:(NSInteger)type {
    UIView *headerView = [[UIView alloc] init];
    headerView.backgroundColor = kBackGrayColor;
    
    if (type == 1) {
        headerView.frame = CGRectMake(0, 0, kIphoneWidth, 40);
    } else {
        headerView.frame = CGRectMake(0, 40 + 216, kIphoneWidth, 40);
    }
    
    UILabel *titleLab = [[UILabel alloc] initWithFrame:CGRectMake(kScreenWidth/2 - 100, 10, 200, 20)];
    titleLab.font = [UIFont systemFontOfSize:15];
    titleLab.textAlignment = NSTextAlignmentCenter;
    titleLab.textColor = kRGBColor(102, 102, 102, 1);
    [headerView addSubview:titleLab];
    
    if (type == 1) {
        titleLab.text = NSLocal(@"请选择日期");
        
        UIButton *button1 = [UIButton buttonWithType:UIButtonTypeCustom];
        [button1 setTitle:NSLocal(@"取消") forState:UIControlStateNormal];
        button1.titleLabel.font = [UIFont systemFontOfSize:16];
        [button1 setTitleColor:kBackColor forState:UIControlStateNormal];
        button1.frame = CGRectMake(15, 0, 60, 40);
        [headerView addSubview:button1];
        [button1 addTarget:self action:@selector(cancel) forControlEvents:UIControlEventTouchUpInside];
        
        UIButton *button = [UIButton buttonWithType:UIButtonTypeCustom];
        [button setTitle:NSLocal(@"确定") forState:UIControlStateNormal];
        button.titleLabel.font = [UIFont systemFontOfSize:16];
        [button setTitleColor:kMainColor forState:UIControlStateNormal];
        button.frame = CGRectMake(kIphoneWidth - 65, 0, 60, 40);
        [headerView addSubview:button];
        [button addTarget:self action:@selector(finishSelect) forControlEvents:UIControlEventTouchUpInside];
        
    } else {
        titleLab.text = @"如没有时间，请选择以下选项";
    }
...
- (void)creatValuePointXs:(NSArray *)xArr withNames:(NSArray *)names
{
    for (int i=0; i<xArr.count; i++) {
        ...
        if (self.frame.size.width > 375) {
            x += ((self.frame.size.width - 375) / 2);
        }
        [self addLabelWithNames:names[i] withPointX:x];
    }
}

- (void)addLabelWithNames:(NSString *)name withPointX:(NSInteger)point_x
{
    CGFloat OrigY = self.frame.size.height/2-10;
    if (self.datePickerStyle == UUDateStyle_YearMonthDay) {
        OrigY = (self.mj_h-self.height)/2-10;
    }
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(point_x, OrigY+self.height, 20, 20)];
    label.text = name;
...
- (NSInteger)numberOfComponentsInPickerView:(UIPickerView *)pickerView
{
    ...
    if (self.datePickerStyle == UUDateStyle_YearMonthDay){
        if (isIPhone5) {
            [self creatValuePointXs:@[@"120",@"200",@"280"]
                      withNames:@[NSLocal(@"年"),NSLocal(@"月"),NSLocal(@"日")]];
        } else {
    ...
    if (self.datePickerStyle == UUDateStyle_MonthDayHourMinute){
        if (isIPhone5) {
            [self creatValuePointXs:@[@"90",@"160",@"230",@"295"]
                      withNames:@[@"月",@"日",@"时",@"分"]];
        } else {
            [self creatValuePointXs:@[@"105",@"175",@"245",@"310"]
                          withNames:@[@"月",@"日",@"时",@"分"]];
        }
        return 4;
    }
    if (self.datePickerStyle == UUDateStyle_HourMinute){
        if (isIPhone5) {
            ...
            [self creatValuePointXs:@[@"160",@"265"]
                          withNames:@[@"时",@"分"]];
        }
        return 2;
    }
